...
import aiosqlite
from config import DB_PATH, MAX_MESSAGES_PER_USER, AUTO_CLEAN_DAYS, ADMIN_ID
...
async def register_user(user_id: int, username: str, full_name: str, business_connection_id: str = None):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            INSERT INTO users (user_id, username, full_name, business_connection_id, joined_at, is_active)
            VALUES (?, ?, ?, ?, ?, 1)
            ON CONFLICT(user_id) DO UPDATE SET
                username=excluded.username,
                full_name=excluded.full_name,
                business_connection_id=COALESCE(excluded.business_connection_id, users.business_connection_id),
                is_active=1
...
async def save_message(user_id: int, chat_id: int, message_id: int, from_user_id: int,
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        is_media = 1 if file_path else 0
        await db.execute("""
        ...
        await db.execute("""
            ...
            SET total_saved = total_saved + 1, 
                ...
                last_active = ? 
            WHERE user_id = ?
        """, (is_media, datetime.now().isoformat(), user_id))
